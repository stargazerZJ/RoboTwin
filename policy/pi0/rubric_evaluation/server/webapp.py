...
def create_app(*, cfg: EvalConfig, manager: EvalManager) -> FastAPI:
    ...
    static_dir = Path("policy/pi0/rubric_evaluation/web/static")
    static_dir.mkdir(parents=True, exist_ok=True)
    app.mount("/static", StaticFiles(directory=str(static_dir)), name="static")

    @app.get("/", response_class=HTMLResponse)
    def index() -> str:
        index_path = Path("policy/pi0/rubric_evaluation/web/index.html")
        if not index_path.exists():
            return "<h1>Missing web UI</h1><p>Expected policy/pi0/rubric_evaluation/web/index.html</p>"
