...
class Pi0(_model.BaseModel):
    ...
    def compute_loss(self,
        ...
        (prefix_out, suffix_out), _, _ = self.PaliGemma.llm([prefix_tokens, suffix_tokens],
                                                         mask=attn_mask,
                                                         positions=positions)
    ...
    def sample_actions(
        ...
        *,
        ...
        # Prepare dummy probs for initialization
        dummy_x_t = noise
        dummy_time = jnp.ones((batch_size,))
        suffix_tokens, suffix_mask, suffix_ar_mask = self.embed_suffix(observation, dummy_x_t, dummy_time)
        ...
        def get_probs_shape():
             _, _, probs = self.PaliGemma.llm([None, suffix_tokens],
                                             mask=full_attn_mask,
                                             positions=positions_suffix,
                                             kv_cache=kv_cache)
             return probs

        probs_shape = jax.eval_shape(get_probs_shape)
        initial_probs = jax.tree.map(lambda x: jnp.zeros(x.shape, x.dtype), probs_shape)
        ...
        def step(carry):
            ...
            (prefix_out, suffix_out), _, probs = self.PaliGemma.llm([None, suffix_tokens],
                                                             mask=full_attn_mask,
                                                             positions=positions,
                                                             kv_cache=kv_cache)
            ...
            return x_t + dt * v_t, time + dt, probs
        ...
        x_0, _, final_probs = jax.lax.while_loop(cond, step, (noise, 1.0, initial_probs))
        return x_0, final_probs
